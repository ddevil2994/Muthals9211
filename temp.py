...
batch_size = 1000  # Number of forwards before taking a break
break_time = 10 * 60  # 10 minutes break in seconds (10 * 60)
skip_messages = 4600  # Number of messages to skip (Start forwarding from 4601st message)
...
async def forward_media():
    try:
        ...
        # Get messages from the source group, and skip the first 4600 messages
        async for message in client.iter_messages(source_group, offset_id=skip_messages):
            if message.media:  # Check if the message has media
                ...
                print(f"Message {forward_count + skip_messages} forwarded")

                # Wait for 1 second between each forward
                await asyncio.sleep(forward_delay)

                # If 1000 messages have been forwarded, take a 10-minute break
                if forward_count % batch_size == 0:
                    print(f"Taking a 10-minute break after {forward_count + skip_messages} forwards.")
